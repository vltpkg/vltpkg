...
Usage:
  vlt query
  vlt query <query> --view=<human | json | mermaid | svg | count>
  vlt query <query> --expect-results=<comparison string>
  vlt query --target=<query> --view=<human | json | mermaid | svg | count>
...
Defaults to listing all dependencies of the project root and workspaces.
  ...
  Options
    ...
    view
      ...
      render the dependency graph as an image and open it. Count outputs the
      number of dependency relationships in the result.

      ​--view=[human | json | mermaid | svg | count]
