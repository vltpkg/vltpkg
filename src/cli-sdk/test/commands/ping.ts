...
t.test('ping all registries when no alias provided', async t => {
  ...
  t.equal(result.length, 3) // default + npm + custom (acme same as default)
  ...
  // Check npm registry (same URL as default, should be skipped)
  // So we should have custom and acme
...
t.test('error on unknown registry alias', async t => {
  try {
    ...
    t.match(err.message, /Unknown registry alias/)
    t.equal(err.cause.found, 'unknown')
    t.ok(Array.isArray(err.cause.wanted))
    t.ok(err.cause.wanted.includes('npm'))
    t.ok(err.cause.wanted.includes('custom'))
  }
})
...
t.test(
  ...
  async t => {
    try {
      ...
      t.match(err.message, /Unknown registry alias/)
      t.equal(err.cause.found, 'unknown')
      t.equal(err.cause.wanted, undefined)
    }
  },
)
