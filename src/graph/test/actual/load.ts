...
t.test('extra parameter in DepID', async t => {
  ...
  const depIdB = joinDepIDTuple(['registry', '', 'b@1.0.0', ':root > #b'])
  const depIdC = joinDepIDTuple(['registry', '', 'c@1.0.0', ':root > #c > #d'])
  ...
  const projectRoot = t.testdir({
    ...
    node_modules: {
      ...
      a: t.fixture(
        'symlink',
        '.vlt/' + depIdA + '/node_modules/a',
      ),
      b: t.fixture(
        'symlink',
        '.vlt/' + depIdB + '/node_modules/b',
      ),
      c: t.fixture(
        'symlink',
        '.vlt/' + depIdC + '/node_modules/c',
      ),
  ...
  const nodeD = graph.nodes.get(joinDepIDTuple(['file', 'packages/d']))
