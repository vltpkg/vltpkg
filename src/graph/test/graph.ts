...
t.test('extra parameter for modifiers', async t => {
  ...
  const projectRoot = t.testdir({ 'vlt.json': JSON.stringify({
    modifiers: {
      ':root > #a': '1.0.0',
      ':root > #a > #c': '1.0.0',
    }
  ...
  t.equal(nodeA?.modifier, ':root > #a', 'node a has the correct modifier')
  ...
  t.equal(nodeC?.modifier, ':root > #a > #c', 'node c has the correct modifier')
  
  // Verify that we can find the nodes in the graph by their modifiers
  const foundNodeA = [...graph.nodes.values()].find(node => 
    node.modifier === ':root > #a'
  )
  
  t.equal(
    foundNodeA,
    nodeA,
    'can find node a by its modifier'
  )
  
  const foundNodeC = [...graph.nodes.values()].find(node => 
    node.modifier === ':root > #a > #c'
  )
  
  t.equal(
    foundNodeC,
    nodeC,
    'can find node c by its modifier'
  )
  ...
  t.equal(
    nodeA?.name,
    'a',
    'node a has the correct name'
  )
  
  t.equal(
    nodeC?.name,
    'c',
    'node c has the correct name'
  )
  ...
  t.ok(
    nodeA?.edgesOut.has('c'),
    'node a has an edge to node c'
  )
