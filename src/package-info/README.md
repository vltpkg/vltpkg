...
`cache`. Both also return `manifest` (the package.json, if the index
has it) and `bindingGyp`, so reify need not read them back from disk.
With `NODE_DEBUG=vlt`, linked / copied / missed counts and the store
hit rate are printed at exit.
