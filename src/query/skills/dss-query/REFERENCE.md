...
Threats: `:malware` (binary, no params), `:squat(sev?)`, `:suspicious`, `:confused`.

Vulnerabilities: `:vuln(sev?)` (alias `:vulnerable`), `:cve(CVE-2023-1234)`,
`:cve(*)`, `:cwe(CWE-79)`, `:severity(level)`.
`:vuln` without params matches severity ≥ medium; `:vuln(critical)` exact-matches;
`:vuln(">=high")` uses comparators. Also matches alerts carrying a `cveId` prop.
