import t from 'tap'
import { removeDanglingEdges, removeNode } from '../../src/pseudo/helpers.ts'
import {getMissingNodeGraph, getSimpleGraph, getMultiWorkspaceGraph} from '../fixtures/graph.ts'
...
t.test('removeDanglingEdges', async t => {
  await t.test('graph with missing nodes', async t => {
    ...
    t.strictSame([...state.partial.edges], [], 'should remove dangling edges')
  })
  ...
  await t.test('graph with no dangling edges', async t => {
    ...
    t.strictSame([...state.partial.edges], [...graph.edges], 'should not remove any edges')
  })
})
...
t.test('removeNode', async t => {
  await t.test('remove a node with incoming edges', async t => {
    const graph = getSimpleGraph()
    const node = graph.nodes.get(joinDepIDTuple(['registry', '', 'a@1.0.0']))!
    const partial = {
      edges: new Set(graph.edges),
      nodes: new Set(graph.nodes.values()),
    }
    ...
    const expectedNodes = [...graph.nodes.values()].filter(n => n !== node)
    t.strictSame([...state.partial.nodes], expectedNodes, 'should remove the node')
  })
