...
t.test(
  ':vulnerable alias works the same as :vuln',
  async t => {
    const getState = (
      query: string,
    ...
    await t.test(
      ...
      async t => {
        ...
        const resVuln = await vuln(
          getState(':vuln(critical)'),
        )
        t.strictSame(
          [...resVulnerable.partial.nodes]
            .map(n => n.name)
            .sort(),
          [...resVuln.partial.nodes]
            .map(n => n.name)
            .sort(),
