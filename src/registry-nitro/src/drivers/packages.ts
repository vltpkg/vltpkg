...
export const definePackagesDriver = <TSchema extends DatabaseSchema>(
  getDb: () => UnifiedDatabase,
  // Schema needs to be passed in since we can't dynamically determine it from the db type
  schema: TSchema,
) =>
  defineDriver(() => {
    const Schema = schema
