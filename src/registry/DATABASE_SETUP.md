...
This command will:
1. Create the initial database tables (`packages`, `tokens`, `versions`)
...
### `packages`
- `name` (TEXT, PRIMARY KEY) - Package name (e.g., "lodash", "@types/node")
- `tags` (TEXT) - JSON string of dist-tags (e.g., `{"latest": "1.0.0"}`)
...
### `versions`
- `spec` (TEXT, PRIMARY KEY) - Package version spec (e.g., "lodash@4.17.21")
...
This means the initial migration ran but the schema update didn't. Run:
```bash
pnpm exec wrangler d1 execute vsr-local-database --file=src/db/migrations/0001_wealthy_magdalene.sql --local --persist-to=local-store --no-remote
```
