...
import { createContext } from './utils/test-helpers.ts'
...
vi.mock('../src/utils/auth.js', async () => {
  ...
  return {
    ...
    getTokenFromHeader: vi
      .fn()
      .mockImplementation(() => 'test-token'),
    getAuthedUser: vi.fn(),
  }
})

// Mock utils
import { getAuthedUser, verifyToken } from '../src/utils/auth.ts'

// Mock functions are used directly via getAuthedUser and verifyToken
...
describe('Access Management API', () => {
  ...
  describe('listPackagesAccess', () => {
    it('should list packages a user has access to', async () => {
      ...
      expect(getAuthedUser).toHaveBeenCalledWith({
        c,
        token: 'test-token',
      })
  ...
  describe('setPackageAccessStatus', () => {
    ...
    it('should reject setting a package to public', async () => {
      ...
      expect(body).toEqual({
        error:
          'Public packages are not supported by this registry at this time',
      })
    })
    ...
    it('should reject if user lacks write access', async () => {
      ...
      expect(body).toEqual({
        error:
          'Unauthorized - you do not have write access to this package',
      })
    })
  })
  ...
  describe('grantPackageAccess', () => {
    ...
    it('should grant read-write access to a user', async () => {
      ...
      expect(body).toEqual({
        success: true,
        message:
          'Added read-write permission for test-user on test-package',
      })
    })
    ...
    it('should reject if admin user lacks user write access', async () => {
      ...
      expect(body).toEqual({
        error:
          'Unauthorized - you do not have permission to manage user access',
      })
    })
  })
