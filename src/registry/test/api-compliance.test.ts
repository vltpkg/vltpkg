...
describe('API Compliance Tests', () => {
  describe('Core API Endpoints', () => {
    ...
    it('should respond to search endpoint with query parameter', async () => {
      const res = await app.request('http://localhost:1337/-/search?text=lodash')
    ...
    it('should return 400 for search endpoint without text parameter', async () => {
      ...
      const data = await res.json() as any
      expect(data.error).toContain('Missing required parameter "text"')
    })
  ...
  describe('Package API Endpoints', () => {
    it('should handle package packument requests', async () => {
      const res = await app.request('http://localhost:1337/npm/lodash')
    ...
    it('should handle scoped package packument requests', async () => {
      const res = await app.request('http://localhost:1337/npm/@types/node')
    ...
    it('should handle package version manifest requests', async () => {
      const res = await app.request('http://localhost:1337/npm/lodash/4.17.21')
    ...
    it('should handle scoped package version manifest requests', async () => {
      ...
      const packageRes = await app.request('http://localhost:1337/npm/@types/node')
      if (packageRes.status === 200) {
        const packageData = await packageRes.json() as any
        const latestVersion = packageData['dist-tags']?.latest
        
        if (latestVersion) {
          const versionRes = await app.request(`http://localhost:1337/npm/@types/node/${latestVersion}`)
    ...
    it('should handle tarball requests', async () => {
      const res = await app.request('http://localhost:1337/npm/lodash/-/lodash-4.17.21.tgz')
    ...
    it('should handle scoped package tarball requests', async () => {
      const res = await app.request('http://localhost:1337/npm/@types/node/-/node-20.0.0.tgz')
  ...
  describe('Local Package Endpoints', () => {
    it('should handle local package requests with fallback to upstream', async () => {
      const res = await app.request('http://localhost:1337/some-nonexistent-package')
    ...
    it('should handle local package version requests with fallback', async () => {
      const res = await app.request('http://localhost:1337/some-nonexistent-package/1.0.0')
      ...
      if (res.status === 302) {
        const location = res.headers.get('location')
        expect(location).toContain('/npm/some-nonexistent-package/1.0.0')
      }
    })
  })
  ...
  describe('Dist-tag Management', () => {
    it('should handle dist-tags requests', async () => {
      const res = await app.request('http://localhost:1337/-/package/lodash/dist-tags')
    ...
    it('should handle scoped package dist-tags requests', async () => {
      const res = await app.request('http://localhost:1337/-/package/@types%2Fnode/dist-tags')
  ...
  describe('Authentication & Authorization', () => {
    it('should require authentication for token operations', async () => {
      const res = await app.request('http://localhost:1337/-/tokens', { method: 'POST' })
    ...
    it('should require authentication for package publishing', async () => {
      const res = await app.request('http://localhost:1337/test-package', { 
        method: 'PUT',
  ...
  describe('Error Handling', () => {
    it('should return 404 for non-existent endpoints', async () => {
      const res = await app.request('http://localhost:1337/non-existent-endpoint')
      
      expect(res.status).toBe(404)
    })

    it('should return 400 for invalid upstream names', async () => {
      const res = await app.request('http://localhost:1337/invalid-upstream/package')
      
      expect(res.status).toBe(400)
      const data = await res.json() as any
      expect(data.error).toContain('Invalid or reserved upstream name')
    })

    it('should return 404 for unknown upstreams', async () => {
      const res = await app.request('http://localhost:1337/unknown-registry/package')
  ...
  describe('Security Audit Endpoints', () => {
    it('should handle security audit requests', async () => {
      const res = await app.request('http://localhost:1337/-/npm/v1/security/advisories/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({})
      })
    ...
    it('should handle upstream-specific audit requests', async () => {
      const res = await app.request('http://localhost:1337/npm/-/npm/v1/security/advisories/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({})
      })
  ...
  describe('Legacy Route Redirects', () => {
    it('should redirect legacy search route', async () => {
      const res = await app.request('http://localhost:1337/-/v1/search?text=lodash')
    ...
    it('should redirect legacy user routes', async () => {
      const res = await app.request('http://localhost:1337/-/npm/v1/user')
    ...
    it('should redirect legacy token routes', async () => {
      const res = await app.request('http://localhost:1337/-/npm/v1/tokens')
