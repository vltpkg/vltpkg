...
import { createDatabaseOperations } from '../src/db/client.ts'
...
describe('Database Operations', () => {
  ...
  beforeEach(() => {
    ...
    dbOps.searchPackages = async (query: string, scope?: string) => {
      const results = Array.from(mockData.packages.values()).filter(
        (pkg: any) => {
          if (scope) {
            return (
  ...
  describe('Tokens', () => {
    ...
    it('should validate user access permissions for token operations', async () => {
      ...
      const { postToken } = await import('../src/routes/tokens.ts')
      ...
      const selfContext = {
        ...
        db: {
          ...
          upsertToken: async (_token, uuid, _scope, authToken) => {
            if (authToken) {
      ...
      const unauthorizedPostResult = await postToken(
        unauthorizedContext,
      )
      expect(unauthorizedPostResult.statusCode).toBe(401)
      expect(unauthorizedPostResult.response.error).toBe(
        'Unauthorized',
      )
  ...
  describe('Versions', () => {
    ...
    it('should insert and retrieve a version', async () => {
      await dbOps.upsertVersion(
        testVersion.spec,
        testVersion.manifest,
