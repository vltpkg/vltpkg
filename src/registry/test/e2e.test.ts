  ...
  lodash: {
    ...
    versions: {
      '4.17.21': {
        ...
        dist: {
          tarball:
            'http://localhost:1337/lodash/-/lodash-4.17.21.tgz',
          shasum: '1ab3cb84daa42f0c9e070f5243eed511d5af2682',
          integrity:
            'sha512-SV/T5Ew+BD1UGwF5Ybo8zCLRU8GlTU7B9qFHWAAS4D2A1etVSUnqGCPHFZtSUrySF5K/NN9JN7MeNdR9SjdkJw==',
        },
      },
      '4.18.0-beta.1': {
        name: 'lodash',
        ...
        dist: {
          tarball:
            'http://localhost:1337/lodash/-/lodash-4.18.0-beta.1.tgz',
  ...
  typescript: {
    ...
    versions: {
      '5.3.3': {
        ...
        dist: {
          tarball:
            'http://localhost:1337/typescript/-/typescript-5.3.3.tgz',
          shasum: '341ee5bbce3effb5ef09b050bf2750addd6f007c',
          integrity:
            'sha512-pXWcraxM0uxAS+tN0AG/BF2TyqmHO014Z070UsJ+pFvYuRwemqUJ8N9vOGi5D4A8XSRmkP9V8WjBadOHgxwVPw==',
        },
      },
    },
  },
...
async function _waitForServer(attempts = 30, delay = 2000) {
  ...
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      console.log(
        `Server responded with ${response.status}, but not ready yet`,
      )
    } catch (err) {
      console.log(
        `Server not ready, attempt ${i + 1}/${attempts}: ${err.message}`,
      )
    }
    await sleep(delay)
  }
...
async function mockFetch(url, options = {}, timeout = TIMEOUT) {
    ...
    if (action === 'dist-tags') {
      if (TEST_PACKAGES[packageName]) {
        ...
        if (options.method === 'PUT' && tag) {
          console.log(
            `Mocking dist-tag add for ${packageName} ${tag}`,
          )
        ...
        if (options.method === 'DELETE' && tag) {
          console.log(
            `Mocking dist-tag remove for ${packageName} ${tag}`,
          )
  ...
  if (parts.length === 2 && TEST_PACKAGES[parts[0]]) {
    ...
    if (pkg.versions[version]) {
      console.log(
        `Mocking manifest response for ${parts[0]}@${version}`,
      )
      return {
        status: 200,
        statusText: 'OK',
  ...
  if (
    parts.length === 3 &&
    parts[1] === '-' &&
    parts[2].includes('.tgz')
  ) {
    const pkgName = parts[0]
    const tarball = parts[2]
    const versionMatch = tarball.match(
      new RegExp(`${pkgName}-(.*)\\.tgz`),
    )

    if (
      versionMatch &&
      TEST_PACKAGES[pkgName]?.versions[versionMatch[1]]
    ) {
      console.log(
        `Mocking tarball response for ${pkgName}@${versionMatch[1]}`,
      )
      return {
        status: 200,
        statusText: 'OK',
...
describe('Registry End-to-End Tests', () => {
  ...
  for (const pkgName of Object.keys(TEST_PACKAGES)) {
    ...
    describe(`${pkgName} package test`, () => {
      ...
      it('should fetch manifest', async () => {
        console.log(
          `Fetching manifest for ${pkg.name}@${Object.keys(pkg.versions)[0]}`,
        )
        ...
        console.log(
          `Successfully fetched manifest for ${pkg.name}@${version}`,
        )
      })

      it('should fetch tarball', async () => {
        console.log(
          `Fetching tarball for ${pkg.name}@${Object.keys(pkg.versions)[0]}`,
        )
        const version = Object.keys(pkg.versions)[0]
        ...
        console.log(
          `Successfully fetched tarball for ${pkg.name}@${version}`,
        )
      })
    })
  }
  ...
  describe('Token validation tests', () => {
    ...
    it('should reject token updates with invalid UUIDs', async () => {
      ...
      expect(data.error).toContain('Invalid uuid')
      console.log(
        'Successfully verified invalid UUID rejection in update',
      )
    })

    it('should reject unauthorized token modifications', async () => {
      console.log(
        'Testing token creation with insufficient permissions',
      )
      const url = `${BASE_URL}/-/npm/v1/tokens`
      ...
      console.log(
        'Successfully verified unauthorized token modification rejection',
      )
    })
    ...
    it('should allow admins to modify any user token', async () => {
      ...
      expect(data.success).toBe(true)
      console.log(
        'Successfully verified admin token modification privilege',
      )
    })
  })
  ...
  describe('dist-tag commands', () => {
    it('should list dist-tags for a package', async () => {
      ...
      const response = await mockFetch(
        `${BASE_URL}/-/package/lodash/dist-tags`,
      )
      expect(response.status).toBe(200)
    ...
    it('should add a dist-tag to a package', async () => {
      ...
      const response = await mockFetch(
        `${BASE_URL}/-/package/lodash/dist-tags/canary`,
        {
          method: 'PUT',
          body: '4.18.0-beta.1',
        },
    ...
    it('should delete a dist-tag from a package', async () => {
      ...
      const response = await mockFetch(
        `${BASE_URL}/-/package/lodash/dist-tags/beta`,
        {
          method: 'DELETE',
        },
      )
    ...
    it('should not delete the latest tag', async () => {
      ...
      const response = await mockFetch(
        `${BASE_URL}/-/package/lodash/dist-tags/latest`,
        {
          method: 'DELETE',
        },
      )
...
const _createMockDrizzle = (mockDb: any) => {
  return mockDb
}
      ...
      if (
        ...
        return {
          body: new ReadableStream({
            start(controller) {
              controller.enqueue(
                new TextEncoder().encode('mock-tarball-content'),
              )
              controller.close()
            },
          }),
...
function createApp() {
  ...
  app.get('/:pkg', async c => {
    ...
    if (TEST_PACKAGES[pkg]) {
      ...
      Object.entries(packageData.versions).forEach(
        ([version, versionData]) => {
  ...
  app.get('/:pkg/-/:tarball', async c => {
    ...
    const versionMatch = new RegExp(`${pkg}-(.*)\\.tgz`).exec(tarball)
    if (
      versionMatch &&
      TEST_PACKAGES[pkg]?.versions[versionMatch[1]]
    ) {
  ...
  app.get('/-/package/:pkg/dist-tags', getPackageDistTagsOriginal)
  app.get(
    '/-/package/:pkg/dist-tags/:tag',
    getPackageDistTagsOriginal,
  )
