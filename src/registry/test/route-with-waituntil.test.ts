...
describe('Route Handler with waitUntil', () => {
   ...
   */
  function createMockContext(options: any = {}): any {
    // Track background task functions
    const backgroundTaskFns: any[] = []
      ...
      runBackgroundTasks: async () => {
        ...
        for (const taskFn of tasks) {
          try {
            ...
            // Background task error ignored in tests
          }
        }
      },
   ...
   */
  async function getPackageMetadata(c: any) {
            ...
            const bgRefresh = async () => {
              try {
                const response = await c.fetch(
                  `https://${CONFIG.DOMAIN}/${packageName}`,
                )
                ...
                // Error refreshing ignored in tests
                resolve()
              }
            }
  ...
  describe('getPackageMetadata', () => {
    ...
    it('should return cached data if fresh', async () => {
      ...
      // Verify
      expect(context.db.get).toHaveBeenCalledWith(
