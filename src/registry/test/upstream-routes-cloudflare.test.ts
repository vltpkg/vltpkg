...
describe('Upstream Routes with Cloudflare Environment', () => {
  ...
  describe('Regular Package Routes', () => {
    it('should fetch regular package metadata', async () => {
      const res = await app.request('http://localhost:1337/npm/lodash', {}, env)
    ...
    it('should fetch specific version of regular package', async () => {
      const res = await app.request('http://localhost:1337/npm/lodash/4.17.21', {}, env)
  ...
  describe('Scoped Package Routes (Unencoded URLs)', () => {
    it('should fetch scoped package metadata with unencoded URL', async () => {
      const res = await app.request('http://localhost:1337/npm/@types/node', {}, env)
    ...
    it('should fetch specific version of scoped package with unencoded URL', async () => {
      ...
      const packageRes = await app.request('http://localhost:1337/npm/@types/node', {}, env)
      ...
      if (packageRes.status === 200) {
        ...
        if (latestVersion) {
          const versionRes = await app.request(`http://localhost:1337/npm/@types/node/${latestVersion}`, {}, env)
    ...
    it('should fetch another scoped package', async () => {
      const res = await app.request('http://localhost:1337/npm/@babel/core', {}, env)
  ...
  describe('URL-Encoded Scoped Packages (Backward Compatibility)', () => {
    it('should still support URL-encoded scoped packages', async () => {
      const res = await app.request('http://localhost:1337/npm/@types%2Fnode', {}, env)
  ...
  describe('Error Handling', () => {
    it('should return appropriate status for non-existent packages', async () => {
      const res = await app.request('http://localhost:1337/npm/this-package-definitely-does-not-exist-xyz', {}, env)
    ...
    it('should return 400 for invalid upstream names', async () => {
      const res = await app.request('http://localhost:1337/invalid-upstream/lodash', {}, env)
    ...
    it('should return 404 for unknown upstreams', async () => {
      const res = await app.request('http://localhost:1337/unknown-registry/lodash', {}, env)
  ...
  describe('Route Disambiguation', () => {
    it('should correctly distinguish scoped packages from version requests', async () => {
      ...
      const scopedRes = await app.request('http://localhost:1337/npm/@types/node', {}, env)
      
      // Test that /npm/lodash/4.17.21 is treated as version request
      const versionRes = await app.request('http://localhost:1337/npm/lodash/4.17.21', {}, env)
      ...
      if (scopedRes.status === 200 && versionRes.status === 200) {
        ...
        expect(Object.keys(scopedData.versions).length).toBeGreaterThan(1) // Should have multiple versions
  ...
  describe('Health Checks', () => {
    it('should respond to ping endpoint', async () => {
      const res = await app.request('http://localhost:1337/ping', {}, env)
