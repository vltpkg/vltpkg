...
vi.mock('../src/routes/packages', () => ({
    ...
    new Response(JSON.stringify({
      ...
      versions: {
        '4.17.21': {
          name: 'lodash',
          version: '4.17.21',
          dist: { tarball: 'http://localhost:1337/npm/lodash/-/lodash-4.17.21.tgz' }
        }
      },
  ...
  getPackageTarball: vi.fn().mockResolvedValue(
...
describe('Upstream Routes Logic Tests', () => {
  ...
  describe('URL Encoding Handling', () => {
    ...
    it('should handle both encoded and unencoded forms', () => {
      ...
      const encodedSegments = decodeURIComponent(encoded).split('/').filter(Boolean)
      ...
      const testCases = [
        {
          ...
          expected: { type: 'scoped-packument', param2: '@types', param3: 'node' }
        },
        {
          path: '/npm/lodash/4.17.21',
          segments: ['npm', 'lodash', '4.17.21'],
          expected: { type: 'regular-manifest', param2: 'lodash', param3: '4.17.21' }
        },
        {
          path: '/npm/@babel/core/7.25.0',
          segments: ['npm', '@babel', 'core', '7.25.0'],
          expected: { type: 'scoped-manifest', scope: '@babel', pkg: 'core', version: '7.25.0' }
        }
      ]
  ...
  describe('Package Name Construction', () => {
    it('should construct package names correctly in getPackageManifest logic', () => {
      ...
      testCases.forEach(({ name, scope, pkg: pkgParam, expected }) => {
        let pkg = pkgParam
  ...
  describe('Upstream Validation Logic', () => {
    it('should validate upstream names correctly', async () => {
      const { isValidUpstreamName } = await import('../src/utils/upstream')
    ...
    it('should get upstream config correctly', async () => {
      const { getUpstreamConfig } = await import('../src/utils/upstream')
  ...
  describe('Response Structure Validation', () => {
    it('should validate packument response structure', () => {
      ...
      expect(Object.keys(mockPackument.versions).length).toBeGreaterThan(0)
    })
