...
describe('waitUntil API', () => {
  it('allows background work to continue after a response is sent', async () => {
    ...
    async function handleRequest(_request, _env, ctx) {
    ...
    expect(response).toEqual({
      message: "Here's your fast response!",
    })
  ...
  it('explains the stale-while-revalidate caching pattern', async () => {
    ...
    // Log what would happen in an actual implementation
    console.log(
      'In a real implementation, a stale-while-revalidate caching strategy would:',
    )
    console.log('1. Check if data exists in cache')
    console.log(
      '2. If it exists but is stale, queue a background task to refresh it',
    )
    console.log(
      '3. Return the cached data immediately, without waiting for the refresh',
    )
...
async function getDataWithBackgroundRefresh(key, db, ctx) {
  ...
  return data;
    ...
    expect(conceptualImplementationPattern()).toContain(
      'ctx.waitUntil',
    )
    expect(conceptualImplementationPattern()).toContain(
      'return cached.data',
