...
/**
 * This class handles the starting and stopping of the server, but
 * the actual response handling is all done in the {@link handlRequest}
 * function.
 */
export class VltServer extends EventEmitter<{
  ...
  async start({ port = 8000 }: VltServerStartOptions = {}) {
    ...
    for (const asset of readdirSync(assetsDir, {
      ...
      const target = resolve(this.publicDir, relative(assetsDir, source))
  ...
  async close() {
    ...
    await Promise.all([
      new Promise<void>((res, rej) =>
        this.server.once('close', (er: unknown) => {
