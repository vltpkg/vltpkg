...
t.test(
  '/host-contexts returns array of TransferData for localhost context',
  async t => {
    const context = getContext(t)
    let responseData = ''
    let statusCode = 0
    ...
    t.ok(response.localhost, 'has localhost context')
    t.ok(Array.isArray(response.localhost), 'localhost is an array')
    t.equal(
      response.localhost.length,
      2,
      'returns data for both projects',
    )

    const firstProject = response.localhost[0]
...
t.test(
  ...
  async t => {
    ...
    const response = JSON.parse(responseData)
    t.equal(response.localhost.length, 2, 'skips failing project')
    t.equal(
      callCount,
      2,
...
t.test(
  ...
  async t => {
    ...
    const response = JSON.parse(responseData)
    t.same(response.localhost, [], 'returns empty array')
  },
)
