...
t.test('/config/update', async t => {
  ...
  ;(context.server as any).config = {
    update: async (key: string, value: string) => {
      ...
      t.equal(key, 'registry', 'should call update with correct key')
      t.equal(
        value,
        'https://updated.registry.com/',
        'should call update with correct value',
      )
    },
  }
  ...
  >('../src/handle-request.ts', {
    '../src/json.ts': {
      read: async (req: IncomingMessage) => {
        ...
        return {
          key: 'registry',
          value: 'https://updated.registry.com/',
        }
      },
      ok: (res: ServerResponse, data: string) => {
        t.equal(res, context.res)
        t.equal(data, 'Config value updated successfully')
        okCalled = true
      },
    },
  })
  await handleRequest(context.req, context.res, context.server)
  t.equal(okCalled, true)
  t.equal(configUpdateCalled, true)
})

t.test('/config/update key not string', async t => {
  const context = getContext(t)
  let errorCalled = false
...
t.test('/config/update value not string', async t => {
  const context = getContext(t)
  let errorCalled = false
...
t.test('/config/update error handling', async t => {
  const context = getContext(t)
  let errorCalled = false
  ...
  >('../src/handle-request.ts', {
    '../src/json.ts': {
      ...
      error: (
        ...
        t.equal(res, context.res)
        t.equal(errType, 'Config update failed')
        t.equal(error, 'Update operation failed')
        t.equal(code, 500)
        errorCalled = true
      },
    },
