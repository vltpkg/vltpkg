...
export const unpack = (
  tarData: ArrayBufferLike | Uint8Array | Buffer = Buffer.allocUnsafe(
    0,
  ),
  target: string,
  ...
  if (isGzip) {
    ...
    /* c8 ignore stop */
    return unpack(gunzipSync(buffer), target, true)
  }
  ...
  while (
    ...
    switch (h.type) {
      ...
      case 'Directory':
        if (!checkFs(h)) continue
        mkdir(h.path.substring('package/'.length))
        break
...
// unpack(readFileSync('./tar-7.0.0.tgz'), './tar')
