...
t.test('unpack into a dir', t => {
  ...
  t.test('buffer', async t => {
    ...
    const tb = raw.buffer.slice(raw.byteOffset, raw.byteOffset + raw.length)
    w.postMessage({ tarData: tb, target: t.testdir(), id: 1 }, [
      tb,
    ])
    check(t, await p, 1)
  })
  ...
  t.test('gzipped', async t => {
    ...
    const tb = raw.buffer.slice(raw.byteOffset, raw.byteOffset + raw.length)
    w.postMessage({ tarData: tb, target: t.testdir(), id: 2 }, [
      tb,
    ])
    check(t, await p, 2)
  })
  ...
  t.test('cannot be main', t => {
    const res = spawnSync(process.execPath, [fileURLToPath( workerScript)], {
      encoding: 'utf8'
    })
