...
unless `store-linker` is set to `hardlink` or `copy`. A package is
copied right before its install scripts run, so they don't change its
files in the store.
...
[`store-linker=copy`](/client/configuring#--store-linkerauto--hardlink--copy--unpack)
