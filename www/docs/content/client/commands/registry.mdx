...
selected by its [`registries`](/client/registries) alias instead of a
full URL.
...
created by [`vlt setup`](/client/commands/setup)).
...
4. With multiple configured registries and no explicit target:
   - on an interactive terminal you are prompted to pick one (the
     [`--default-registry-alias`](/client/registries#bare-specs-and---default-registry-alias)
     is pre-selected);
...
- [`vlt setup`](/client/commands/setup)
