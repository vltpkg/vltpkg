...
Use [DSS queries](/client/selectors) to target specific packages:
...
Filter packages using a [DSS query](/client/selectors) selector.
...
- [`vlt exec`](/client/commands/exec) - Run arbitrary commands with
  enhanced PATH
- [`vlt run-exec`](/client/commands/run-exec) - Intelligently choose
  between run and exec
- [Workspaces](/client/selectors) - Dependency Selector Syntax query
  language reference
- [Workspaces](/client/workspaces) - Working with monorepos
- [Affected Dependencies](/client/affected) - Scoping a run to what a
