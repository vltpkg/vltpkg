...
<Code
  code="$ vlt [<options>] [<cmd> [<args> ...]]"
  title="Terminal"
  lang="bash"
/>

More documentation available at
[https://docs.vlt.io](https://docs.vlt.io)
...
Object type values given via `VLT_*` environment variables or on the
command line are merged key by key over the config files, command line
winning over environment. Other values replace them. List values in
the environment are newline-delimited, e.g.
`VLT_REGISTRIES=$'a=https://a/\nb=https://b/'`.
...
protocol, see the [Catalogs documentation](/client/catalogs).
...
set; [`vlt setup`](/client/registries#getting-started-with-vlt-setup)
or [`vlt login`](/client/commands/login) will configure one for you.
See [Registries](/client/registries).
...
[`vlt setup`](/client/registries#getting-started-with-vlt-setup)),
...
<Code
  code="​​$ vlt --registries loc=http://reg.local install foo@loc:foo@1.x"
  title="Terminal"
  lang="bash"
/>
...
Aliases given on the command line or in `VLT_REGISTRIES` merge with
the configured ones, later sources winning per key. Pass
[`--save-config`](#--save-config) to `vlt install`, `vlt add` or
`vlt update` to save them to `vlt.json`. See
[Registries](/client/registries#saving-registries-given-to-install).
...
For example, `--scoped-registries @acme=https://registry.acme/` would
...
How packages are placed in `node_modules`. Every linker but `unpack`
fills a global store under `cache` in the background, then places
packages from it.

- auto: Default. On Linux, hardlink from the global store, copying any
  file that cannot be linked. Elsewhere, same as `unpack`: a hardlink
  is the slowest way to place a file on APFS, so macOS and Windows
  unpack by default.
- hardlink: Hardlink from the global store on every platform.
- copy: Copy from the global store. Use this when editing files in
  `node_modules` in place, since a hardlink shares its content with
  every project.
- unpack: Unpack each package tarball, skipping the global store.

A package is copied right before its install scripts run, so they
don't change its files in the store. An invalid `VLT_STORE_LINKER`
warns and uses `unpack`. See
[Global store](/client/commands/cache#global-store).

Valid options: "auto", "hardlink", "copy", "unpack"

### `--brotli-tarballs`

Resolve to the Brotli (`.tar.br`) tarball when the registry advertises
one for a version, transferring far less than the gzip `.tgz`. On by
default; `--no-brotli-tarballs` always resolves to the `.tgz`.

A `.tar.br` is its own artifact, not a re-encoding of the `.tgz`: it
hashes differently, and the registry labels it with its own RFC 9530
`Repr-Digest`, which is what verifies the download. So a version
installed this way records _that_ hash in `vlt-lock.json`, alongside a
flag marking which artifact the entry describes.

An existing lockfile keeps whichever artifact it already pinned, the
same as any other resolved URL — set `--no-brotli-tarballs` before the
versions are resolved to pin `.tgz` files instead.

Registries that advertise no alternates are unaffected.

### `--tag=<tag>`

Default `dist-tag` to install

### `--before=<date>`

Do not install any packages published after this date
...
Specify which configuration to show or operate on when running
`vlt config` commands.

For read operations (`get`, `pick`, `list`): `all` shows merged
configuration from both user and project files (default).

For write operations (`set`, `delete`, `edit`): defaults to `project`.
`user` shows/modifies only user-level configuration, `project`
shows/modifies only project-level configuration.

Valid options: "all", "user", "project"
...
```
"cache", "config", "exec", "exec-local",
"help", "init", "install", "login", "logout",
"list", "ls", "pkg", "query", "run-exec", "run", "serve",
"token", "uninstall", "exec-cache", "whoami"
```

### `--package=<p>`

When running `vlt exec`, this allows you to explicitly set the package
to search for bins. If not provided, then vlt will interpret the first
argument as the package, and attempt to run the default executable.
...
The root directory to use for the dashboard browser-based UI. If not
...
Save the registry and git host options given on the command line or
via `VLT_*` env (`--registry`, `--registries`,
`--default-registry-alias`, `--scoped-registries`, `--jsr-registries`,
`--git-hosts`, `--git-host-archives`) to the project `vlt.json` (or
the user one with `--config=user`) after a successful `install`, `add`
or `update`. Values a config file already has are left alone. See
[Registries](/client/registries#saving-registries-given-to-install).
