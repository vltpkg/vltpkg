...
Unlike npm, **vlt has no default registry**. Commands that resolve or
...
which configures your account's registry aliases for you.
...
export `VLT_REGISTRIES`. See [Registries](/client/registries).
...
| `npm audit`                    | `vlt query ':malware'`                     | More powerful; see [Security](/client/security) |
| `npm config set <key>=<val>`   | `vlt config set <key>=<val>`               | Writes to `vlt.json`                            |
| `npm overrides`                | [Graph Modifiers](/client/graph-modifiers) | DSS-based, more precise                         |
...
$ vlt config set scoped-registries=@mycompany=https://npm.mycompany.com/`}
  title="Terminal"
  lang="bash"
/>
...
vlt also supports [named registry aliases](/client/registries), which
are more explicit than scope-based mapping. This is a vlt-specific
feature:
...
See [Authentication](/client/auth) for full details.
...
See [`vlt build`](/client/commands/build) for full details.
...
See [Workspaces](/client/workspaces) for full details.
...
vlt uses [Graph Modifiers](/client/graph-modifiers) in `vlt.json`,
which are powered by DSS selectors and offer more precise targeting.
