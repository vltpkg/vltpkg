...
instead to configure your account's registry aliases. See
...
| `pnpm <script>`                  | `vlt run <script>`              | See [fallback-command](/client/configuring#--fallback-commandcommand) |
...
| `pnpm list`                      | `vlt list`                      |                                                                       |
| `pnpm why <pkg>`                 | `vlt query '#<pkg>'`            | DSS query; see [Selectors](/client/selectors)                         |
| `pnpm install --frozen-lockfile` | `vlt install --frozen-lockfile` |                                                                       |
| `pnpm audit`                     | `vlt query ':malware'`          | More powerful; see [Security](/client/security)                       |
...
direct support for [catalogs](/client/catalogs) with compatible
syntax.
...
See [Catalogs](/client/catalogs) for full details.
...
[Graph Modifiers](/client/graph-modifiers) in `vlt.json`.
...
- **`pnpm.peerDependencyRules`** — vlt handles peer dependencies
  automatically with [context isolation](/client/peer-dependencies)
