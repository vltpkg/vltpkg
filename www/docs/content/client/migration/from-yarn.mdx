...
| `yarn <script>`                  | `vlt run <script>`              | See [fallback-command](/client/configuring#--fallback-commandcommand) |
...
| `yarn why <pkg>`                 | `vlt query '#<pkg>'`            | DSS query; see [Selectors](/client/selectors)                         |
...
are more explicit than scoped registries. Dependencies reference a
named registry directly, removing ambiguity:
...
versions. vlt uses [Graph Modifiers](/client/graph-modifiers) in
`vlt.json`.
...
- **Constraints** — Use vlt's [DSS query system](/client/selectors)
  for dependency policy enforcement
