...
Note that registry nodes are matched against the resolved registry
URL. Since vlt has no [default registry](/client/registries), a node
whose spec carries no registry has no hostname to compare and will not
match.
...
- [`:registry()`](/client/selectors/pseudo-classes/registry) — match
  by registry configuration name
- [`:type()`](/client/selectors/pseudo-classes/type) — match by
  package type (registry, git, file, etc.)
