...
<Code code="$ vlt ls
$ vlt ls <query> --view=[human | json | mermaid | gui]" title="Terminal" lang="bash" />
...
<Code code={`$ vlt ls '[name="@scoped/package"] > *'`} title="Terminal" lang="bash" />

List all peer dependencies of all workspaces

<Code code={`$ vlt ls '*.workspace > *.peer'`} title="Terminal" lang="bash" />
