...
<Code
  code="$ vlt ls
$ vlt ls <query> --view=[human | json | mermaid | png | svg | pdf | count]"
  title="Terminal"
  lang="bash"
/>
...
Output format. Defaults to human-readable or json if no tty. Use
`png`, `svg`, or `pdf` to render the dependency graph as an image and
