...
The graph being queried is the one vlt installed, read from
`node_modules/.vlt-lock.json`. Run `vlt install` first: in a project
installed by another client there is no such graph to query. `:host()`
queries are the exception, since they load their graph from somewhere
else.
