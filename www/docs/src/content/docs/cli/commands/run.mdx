...
Use [DSS queries](/cli/dependency-selectors) to target specific packages:
...
Filter packages using a [DSS query](/cli/dependency-selectors) selector.
...
- [`vlt exec`](/cli/commands/exec) - Run arbitrary commands with enhanced PATH
- [`vlt run-exec`](/cli/commands/run-exec) - Intelligently choose between run and exec
- [Package Scripts](/cli/scripts) - Learn more about package.json scripts
- [Workspaces](/workspaces) - Working with monorepos
