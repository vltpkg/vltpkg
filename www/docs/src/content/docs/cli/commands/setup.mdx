...
3. Optionally authenticates against those registries via the browser
   web-login flow. Each registry has its own credentials, so the
   browser opens once per registry — first for the public `npm`
   mirror, then for your private `main` registry.
