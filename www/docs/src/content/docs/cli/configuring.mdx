...
the configured ones, later sources winning per key. When given to
`vlt install`, `vlt add` or `vlt update`, they (and `--registry`,
`--scoped-registries`, `--jsr-registries`, `--git-hosts`,
`--git-host-archives`) are saved to the project `vlt.json`, or the
user one with `--config=user`, unless a config file already has them.
See [Registries](/cli/registries#saving-registries-given-to-install).
