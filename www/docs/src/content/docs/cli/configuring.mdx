...
comes from `registries.npm` or, failing that, `--registry`. Configure
one (for example via
...
The `npm:` prefix is **not** pre-registered to any URL — it falls back
to `--registry` when it has no entry here, and is otherwise configured
(for example with `vlt setup`). The built-in `gh:` and `jsr:` prefixes
are pre-registered but remain overridable here.
