...
```
cache, config, exec, exec-local, gui, help, init, install, login, logout, list, ls, pkg, query, run-exec, run, token, uninstall, exec-cache, whoami
```
...
"gui", "help", "init", "install", "login", "logout",
"list", "ls", "pkg", "query", "run-exec", "run", "token",
"uninstall", "exec-cache", "whoami"
```
