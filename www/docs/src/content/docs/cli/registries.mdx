...
Registry and git host options passed on the command line or via
`VLT_*` env to `vlt install`, `vlt add` or `vlt update` are saved to
the project `vlt.json` after a successful install, so a fresh clone
installs with a plain `vlt install`:

<Code
  code="​​$ vlt install --registries loc=http://reg.local foo@loc:foo@1.x"
  title="Terminal"
  lang="bash"
/>
