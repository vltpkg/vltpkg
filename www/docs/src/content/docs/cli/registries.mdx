...
- A different value for the same key in the target file (or its
  ...
  Change it with `vlt config set registries.loc=<url>` first. An
  invalid alias name or a missing URL is an `ECONFIG` error too.
...
- A `vlt install` run by a vlt script (`vlt run`, `vlt exec`, ...)
  only saves what was given to the parent vlt or set for the child,
  not config the parent read from its own files.
