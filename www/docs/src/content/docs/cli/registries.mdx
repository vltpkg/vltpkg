...
**no longer built-in** — it has no baked-in URL, and resolves through
`registries.npm` or, failing that, the `registry` scalar (see
[The `npm:` alias](#npm) below). All built-in aliases are
...
Because the `npm` alias has no built-in URL, a fresh project with
neither `registry` nor `registries.npm` set cannot resolve bare specs
— the guided way to configure one is `vlt setup`.
...
If `registries.npm` is unset, the `npm:` alias falls back to the
`registry` scalar, so alias specs like `foo@npm:bar@1.2.3` — which
published packages use in their own dependencies — still resolve when
only `--registry` is configured.

With neither set, bare specs like `foo@1.2.3` and `foo@npm:bar@1.2.3`
have no registry to resolve against and will fail with the
