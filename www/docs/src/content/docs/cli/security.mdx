...
The `:malware` selector is **binary** — it takes no parameters. A node
matches if it has at least one `malware` or `gptMalware` alert.

<Code
  code={`# Find any malware
$ vlt query ':malware'
...
> [!NOTE] `gptMalware` alerts are AI-generated. For severity-based
> filtering of vulnerability alerts (including `gptSecurity` and
> `gptAnomaly`), use the `:vuln` selector below.

## Vulnerability Detection

### Severity-Based Filtering

Use `:vuln` for severity-based vulnerability detection. VLT maps
vulnerability alerts into four severity levels:

- **Critical (0)** — `criticalCVE`
- **High (1)** — `cve`
- **Medium (2)** — `potentialVulnerability`, `gptSecurity`
- **Low (3)** — `mildCVE`, `gptAnomaly`

Alerts carrying a `cveId` prop (e.g. `socketUpstreamVulnerability`)
are also matched, using their `severity` field for level comparison.

<Code
  code={`# Find critical vulnerabilities only
...
# Find medium or lower vulnerabilities
...
# Parameterless :vuln matches severity >= medium

$ vlt query ':vuln'`} title="Terminal" lang="bash" />
