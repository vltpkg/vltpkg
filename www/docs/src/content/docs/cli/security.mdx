...
Lower numbers are more severe. When using comparators, prefer the
numeric form -- it makes the direction explicit instead of relying on
remembering which word maps to which number:
...
$ vlt query ':severity("<=1")'`} title="Terminal" lang="bash" />
