...
For a complete vulnerability audit with severity filtering and
detailed reporting, use the [`vlt audit`](/cli/commands/audit)
command:

<Code code="$ vlt audit" title="Terminal" lang="bash" />

Alternatively, you can perform custom security checks using
`vlt query` with security selectors as shown below.
...
The `:malware` selector is **binary** — it takes no parameters. Any
matching result may have alerts or CVEs assigned to it.
...
Use `:vuln` for severity-based vulnerability detection with four
