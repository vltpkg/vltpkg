...
- `:path("<glob>")` Matches packages based on their file path relative to
  the project root using glob patterns. Path patterns must be quoted strings. Examples:
  - `:path("./node_modules/*")` - Matches all dependencies in node_modules
  - `:path("**/react")` - Matches all dependencies named "react" anywhere
  - `:path("./workspace/foo")` - Matches a specific workspace dependency
  - `:path("packages/**")` - Matches all packages in the packages directory
