...
- `:path("<glob>")` Matches workspace & file packages based
  on their file path relative to the project root using glob
  patterns. Path patterns must be quoted strings. Examples:
