...
<Aside type="tip">
  `:malware` matches only high-confidence malware alerts (`malware` and
  `gptMalware`). For broader vulnerability detection with configurable
  severity levels, use [`:vuln`](#vulnerable--vuln).
</Aside>
