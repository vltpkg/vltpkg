...
- **Self-Hosted**: Deploy VSR on your own cloud infrastructure with
  the new `vsr deploy` command
- **Managed Service**: Use our hosted VSR service for hassle-free
  registry management
- **Hybrid**: Combine self-hosted and managed components for maximum
  flexibility

Deploy to production in minutes with a single command:

```bash
vsr deploy --env=prod
```

See our [Deployment Guide](/registry/deployment) for complete setup
instructions.
