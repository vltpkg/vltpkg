...
const readCookie = (): PackageManager => {
  const match = new RegExp(`(?:^|;\\s*)${COOKIE}=([^;]+)`).exec(
    document.cookie,
  )
...
export const usePreferredPackageManager = () => {
  const preferredPm = useSyncExternalStore(
    subscribe,
    readCookie,
    () => DEFAULT,
  )
...
export const usePackageManagerTabs = (
  labels: (string | undefined)[],
) => {
  const [picked, setPicked] = useState('0')
  const { preferredPm, setPreferredPm } = usePreferredPackageManager()
  const pmTab = labels.findIndex(
    label => label?.toLowerCase() === preferredPm,
  )
