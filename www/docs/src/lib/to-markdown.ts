 ...
 */
export async function toMarkdown({ docId, request }: ToMarkdownProps): Promise<string> {
  // Get the doc entry
  ...
  turndownService.addRule('expressiveCodeBlock', {
    filter: (node) => {
      return node.nodeName === 'DIV' && node.className?.includes('expressive-code')
    },
    replacement: (content) => {
      // Extract code from the complex HTML structure
      const codeMatch = content.match(/```[\s\S]*?```/m)
