 ...
 */
const fixHeadings = (page, node, index, parent) => {
  if (
    page === '/index.md' &&
    node.depth === 2 &&
  ...
  if (page.endsWith(`/${entryFileName}`) && node.depth === 1) {
...
function MarkdownFixes() {
  ...
  this.compiler = (tree, file) => {
    ...
    visit(tree, 'link', node => fixLinks(page, node))
    visit(tree, 'heading', (node, index, parent) =>
      fixHeadings(page, node, index, parent),
    )
